...
namespace RopeMinikit
{
    ...
    public class Rope : MonoBehaviour
    {
        ...
        public struct CollisionSettings
        {
            ...
            [Tooltip(
                "Check and respond to collisions on every n:th rope particle. A value of one will make every simulated particle react to collisions, a value " +
        ...
        public int GetParticleIndexAt(float distance)
        {
            ...
            return math.clamp((int)(distance / _measurements.particleSpacing + 0.5f), 0, positions.Length - 1);
        }
        ...
        protected void ComputeRealCurve(Allocator allocator, out RopeMeasurements measurements, out NativeArray<float3> points)
        {
            ...
            measurements = new RopeMeasurements()
            {
        ...
        protected void PrepareRigidbodyConnections()
        {
            ...
            {
                var doubleLength = particleTargets.Length * 2;
        ...
        protected void ApplyRigidbodyConnectionFeedback()
        {
            ...
            for (int i = 0; i < liveRigidbodyConnections.Count; i++)
            {
                var c = liveRigidbodyConnections[i];
        ...
        {
            if (!initialized || !simulation.enabled)
            {
                return;
            }
        ...
        public void OnDrawGizmos()
        {
            ...
            ComputeRealCurve(Allocator.Temp, out RopeMeasurements measurements, out NativeArray<float3> points);
